...
app.get('/api/:room_code/persons',(req,res)=>{
    ...
    //console.log('Navbar',room[req.params.room_code].persons);
...
var person = room[req.params.room_code].persons.find(p=> p.handle == (req.params.handle));
if(!person){
...
//    if(req.params.room_code!=roomkey){
//        res.status(404).send("No such room Found!");
//        return ;
//console.log("getting")
var ob = {
   ...
   {
        person = room[req.params.room_code].persons.find(p=> p.select ==(req.params.select));
        if(person){
...
//id: room[req.params.room_code].persons.length+1,
...
app.delete('/api/:room_code/persons/:handle/:select',(req,res)=>{
 //console.log('delete',req.params.handle,' ',persons);
...
if(len == 0 || (len!=0 && data.handle!=room[data.room_code].chathistory.pre_chat)){
